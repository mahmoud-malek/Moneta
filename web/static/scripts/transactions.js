        ...
        if (data.success) {
          ...
          transactionItem.innerHTML = `
					<div class="transaction-info">
						<span>${new Date(data.transaction.date).toLocaleDateString()}</span>
						<span>${data.transaction.category}</span>
						<span>${data.transaction.type}</span>
					</div>
					<div class="transaction-amount ${data.transaction.type.toLowerCase()}">
						<span>${data.transaction.type === 'Income' ? '+' : '-'}$${data.transaction.amount}</span>
					</div>
    ...
    fetch(`/api/v1/transactions/${transactionID}`, {
      ...
      .then(data => {
        if (data.success) {
          ...
          transactionItem.querySelector('.transaction-info span:first-child').textContent = new Date(data.transaction.date).toLocaleDateString('en-GB');
          transactionItem.querySelector('.transaction-info span:nth-child(2)').textContent = categories[data.transaction.category_id];
          transactionItem.querySelector('.transaction-info span:last-child').textContent = data.transaction.type;
